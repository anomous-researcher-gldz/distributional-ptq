...
for m in sorted(set(grp2)):
    sel=grp2==m
    per_config_gatepass[m] = round(float(base[sel].mean()*100), 1)
    print(f"   {m:14s}: {base[sel].mean()*100:5.1f}%  (n={sel.sum()})")

# ---------- persist headline numbers so they trace to a committed artifact ----------
out = {
    "description": "Author-response threshold-robustness + boundary-AUC numbers, "
                   "derived deterministically (seed 0) from committed per-layer stats "
                   "in results/S4-cross-model-layer-analysis/. Generated by "
                   "cross_arch_generalization/scripts/threshold_robustness_dispatch.py.",
    "n_layers": int(len(rows)), "n_configs": len(files),
    "boundary_auc": {
        "label": "activation outlier gate (a_gate)",
        "features": ["a_frac3", "log10(a_kurt)", "abs(a_skew)"],
        "protocol": "leave-one-architecture-out (family held out)",
        "per_family": {k: (None if v is None else round(float(v), 3))
                       for k, v in auc_fam.items()},
        "mean": round(float(mean_fam), 3),
        "leave_one_config_out_mean": round(float(mean_cfg), 3),
    },
    "class_balance": {"fire_pct": round(float(ya.mean()*100), 1),
                      "skip_pct": round(float((1-ya.mean())*100), 1),
                      "n": int(len(ya))},
    "threshold_jitter": {"n_draws": 200, "bounds_perturbed": 5, "pct": 20,
                         "mean_agreement_pct": round(float(agree.mean()*100), 1),
                         "min_agreement_pct": round(float(agree.min()*100), 1),
                         "p5_agreement_pct": round(float(np.percentile(agree, 5)*100), 1),
                         "changed_pct": round(float((1-agree.mean())*100), 1)},
    "weight_gatepass_pct_per_config": per_config_gatepass,
    "gain_regression_r": {"frozen_1feature_loco": round(float(r_line_loo), 3),
                          "learned_3feature_loco": round(float(r_multi), 3),
                          "insample_frac3_llama_paper": round(float(r_line), 3)},
}
OUT = os.path.join(os.path.dirname(__file__), "..", "results",
                   "threshold_robustness_results.json")
