...
class TDQuantModel:
    ...
    def optimize_parameters(self, current_iter):
        ...
        start = time.time() 
        gc.collect()
        # Ask PyTorch to release cached memory back to the CUDA driver
        torch.cuda.empty_cache()

        # (Optional) Clean up inter-process handles
        torch.cuda.ipc_collect()
        # print("q forward pass didn't go through")
        self.output_Q = self.net_Q(self.lq)
       
        if current_iter==50: 
            time_taken = (time.time() - start)/16 
            print('time taken q 4bit {}'.format(time_taken),flush=True)
            print(self.lq.shape,flush=True)
        with torch.no_grad():
            start = time.time() 
            self.output_F = self.net_F(self.lq)
            if current_iter==50: 
                time_taken = (time.time() - start)/16 
                print('time taken f {}'.format(time_taken),flush=True)
                print(self.lq.shape,flush=True)
        l_total = 0
        ...
        #l_total = torch.clamp(l_total,min=1e-6, max=1e3)
        
        l_total.backward()
    
        torch.nn.utils.clip_grad_value_(self.net_Q.parameters(), clip_value=1.0)
    ...
    def save_network(self, net, net_label, current_iter, param_key="params"):
        ...
        def analyze_save_dict_noprune(save_dict: dict, topk: int = 10) -> None:
        ...
        def analyze_save_dict(save_dict: dict, topk: int = 10) -> None:
            grand = defaultdict(int)

            print("=" * 90,flush=True)
            for model_name, state_dict in save_dict.items():
                # 1) Collect all scopes that have any quantize
                quant_scopes = set()
                for k in state_dict.keys():
                    if is_quantizer_key(k):
                        scope = quant_scope_from_key(k)
        ...
        def torch_save_size_mb(obj, use_zip=True):
            ...
            return buf.getbuffer().nbytes / (1024**2)  # MiB
        ...
        while retry > 0:
            printed=0 
            try:
                #this is where to do size eval 
                if printed==0: 
                    size_mb = analyze_save_dict(save_dict)
                    print(f"{size_mb:.2f} 2bit MiB ours+ 40% pruning",flush=True)

                    size_mb = analyze_save_dict_noprune(save_dict)
                    print(f"{size_mb:.2f} 2bit MiB ours+ no pruning",flush=True)

                    size_mb = analyze_save_dict_2dquant(save_dict)
                    print(f"{size_mb:.2f} 2bit MiB 2dquant",flush=True)
