...
Post-training quantization (PTQ) methods are typically architecture-specific. Yet two distributional pathologies they target, dense cores with sparse outliers and input-conditioned activation clusters, recur across model families. We argue that for these pathologies the unit of selection should be the tensor distribution, not the architecture. We propose two composable primitives, dispatched from one calibration pass by a two-statistic diagnostic: Dual-Band Affine Folding (DBAF) where a per-tensor outlier gate fires, and Prompt-Conditioned Scale Anchoring (PCSA) where a per-prompt compactness ratio does. The diagnostic is a selection heuristic, not a calibrated predictor. Four prediction-then-outcome checks are consistent with the dispatch: a rotation cliff on a six-host LLaMA-3-8B W4A4 matrix, cross-family transfer to Qwen-2.5-7B, per-site gating on the KV-cache and SAM-B (+5 percentage points NIAH at 8k, +4.8 mAP), and a negative SwinIR-×3 prediction at training-free W4A16. Both primitives are training-free and calibrate in seconds. The gate’s boundary is learnable, not merely hand-set: a logistic dispatcher trained with each model family held out reproduces it at mean AUC 0.962. Calibrated once on LLaMA-3-8B, the same thresholds dispatch across seven model families in six domains; off distribution the gate errs toward skipping.
